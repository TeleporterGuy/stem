...
function registerExecTool(pi) {
  pi.registerTool({
    ...
    description:
      ...
      '-Command "..."` so a broken profile.ps1 cannot block the run. Use this to drive CLIs, git, build ' +
      'tools, or quick scripts — NOT to read files (use the dedicated read/grep/find/ls tools). By default ' +
      'the command runs in an isolated Stem workspace folder; pass `cwd` only when it must run in a ' +
      'specific existing directory. Folders connected read-only are blocked entirely. Safe commands run ' +
      'immediately; others are screened by an automatic safety check and may pause for the user\'s ' +
      'approval (denied automatically in scheduled runs — prefer simple, clearly-safe commands there). ' +
      'Always quote arguments containing special characters (&, ?, ;, spaces) — e.g. agent-browser open ' +
      '"https://example.com/watch?v=x&t=1" — an unquoted & or ; changes what the shell runs and forces the ' +
      'approval path. Output is captured with the exit code and truncated past 64KB per stream; default ' +
      'timeout 60s (max 300s via `timeout_ms`).',
    parameters: {
      type: 'object',
      properties: {
